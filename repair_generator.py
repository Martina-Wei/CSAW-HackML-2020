...
class BD():
    ...
    def get_pca_treshold(self, vector_x, y):
        for i in range(N_class):
            idx = (y==i)
            matrix = np.dot(self.mean_vector.get(i), np.transpose(vector_x[idx]))
    ...
    def pca_filter(self, xs, ys):
        ...
        for i, (x, y) in enumerate(zip(xs, ys)):
            ...
            if np.linalg.norm(temp) < np.min(self.treshold[y]) or  np.linalg.norm(temp) > np.max(self.treshold[y]):
                result[i] = True
        return result
